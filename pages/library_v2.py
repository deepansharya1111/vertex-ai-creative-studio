...
class PageState:
    ...
    error_filter: str = "no_errors"  # "all", "no_errors", "only_errors"
        ...
        with me.box(
            ...
            if (
                ...
                for item in media_items:
                    ...
                    # Use thumbnail if available, otherwise fallback to raw asset
                    thumbnail_uri = getattr(item, 'thumbnail_uri', None)
                    display_gcs = thumbnail_uri if thumbnail_uri else gcs_uri

                    # Construct the display URL on the fly.
                    https_url = create_display_url(display_gcs) if display_gcs else ""
