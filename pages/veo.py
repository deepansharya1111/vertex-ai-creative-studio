...
from state.state import AppState, check_feature_access
...
def veo_page():
    ...
    with page_scaffold(page_name="veo"):  # pylint: disable=not-context-manager
        if not check_feature_access("video"):
            with me.box(style=me.Style(padding=me.Padding.all(40), display="flex", flex_direction="column", align_items="center", justify_content="center", height="100%")):
                me.text("Access Denied", type="headline-4")
                me.text("You do not have permission to access the Video feature or your access has expired.")
            return
        veo_content(state)
...
def on_click_veo(e: me.ClickEvent):  # pylint: disable=unused-argument
    ...
    # --- Quota Check ---
    amount = max(1, state.video_count)
    from state.state import check_quota
    if not check_quota("video", amount):
        state.error_message = f"Quota exceeded. You do not have enough quota to generate {amount} video(s). Please contact your admin for this."
        state.show_error_dialog = True
        yield
        return

    try:
        from services.access_service import increment_user_usage
        increment_user_usage(app_state.user_email, "video", amount)
        app_state.video_usage += amount
    except Exception as e:
        print(f"Failed to pre-increment usage: {e}")
    ...
    while state.job_status in ["pending", "processing", "created"]:
        ...
        try:
            ...
            if state.job_status == "complete":
                ...
                # Refund quota
                try:
                    from services.access_service import decrement_user_usage
                    decrement_user_usage(app_state.user_email, "video", amount)
                    app_state.video_usage = max(0, app_state.video_usage - amount)
                except Exception as refund_ex:
                    print(f"Failed to refund usage: {refund_ex}")
