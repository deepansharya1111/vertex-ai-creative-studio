...
def sidenav(current_page: Optional[str]):
    ...
    top_nav_items = [p for p in WELCOME_PAGE if p.get("route") and p.get("align") != "bottom" and is_route_allowed(p.get("route"))]
    bottom_nav_items = [p for p in WELCOME_PAGE if p.get("route") and p.get("align") == "bottom" and is_route_allowed(p.get("route"))]
            ...
            with me.box(style=MENU_BOTTOM):
                if app_state.sidenav_open:
                    with me.box(
                        ...
                        me.text("Quota Usage", style=me.Style(font_size="0.75rem", font_weight="bold", color=me.theme_var("on-surface-variant")))
                        
                        def format_quota(q):
                            return "∞" if q == 0 else str(q)
                            
                        usage_text = f"Img {app_state.image_usage}/{format_quota(app_state.image_quota)} | Vid {app_state.video_usage}/{format_quota(app_state.video_quota)} | Aud {app_state.audio_usage}/{format_quota(app_state.audio_quota)}"
                        me.text(usage_text, style=me.Style(font_size="0.65rem", color=me.theme_var("on-surface-variant")))
