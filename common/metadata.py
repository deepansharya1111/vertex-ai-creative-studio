...
def add_media_item_to_firestore(item: MediaItem):
    ...
    try:
        ...
        # Trigger background thumbnail generation if applicable
        # Ensure we have a valid GCS URI, it is complete, and doesn't already have a thumbnail.
        if getattr(item, 'gcsuri', None) and not getattr(item, 'thumbnail_uri', None) and getattr(item, 'status', '') == "complete":
            try:
                from common.thumbnail_utils import trigger_thumbnail_generation
                mime_type = getattr(item, 'media_type', None)
                if not mime_type:
                    # Fallback inference
                    mime_type = "video/mp4" if ".mp4" in item.gcsuri.lower() else "image/png"
                trigger_thumbnail_generation(item.id, item.gcsuri, mime_type)
            except Exception as e:
                logger.error(f"Failed to trigger thumbnail generation: {e}")
...
def get_media_for_page(
    ...
    """Fetches a paginated and filtered list of media items from Firestore using native server-side queries."""
    try:
        query = db.collection(config.GENMEDIA_COLLECTION_NAME)

        # Apply user email filter using native WHERE clause
        if filter_by_user_email:
            query = query.where("user_email", "==", filter_by_user_email)

        # Apply type filters using native WHERE clauses (prefix matching using inequalities)
        if type_filters and "all" not in type_filters:
            # We can only perform one inequality filter per query.
            # If multiple types are selected, we must fetch all and filter client side.
            # However, typically only one type tab is active at a time in the UI.
            if len(type_filters) == 1:
                t = type_filters[0]
                if t == "videos":
                    query = query.where("mime_type", ">=", "video/").where("mime_type", "<", "video0")
                elif t == "images":
                    query = query.where("mime_type", ">=", "image/").where("mime_type", "<", "image0")
                elif t in ("music", "audio"):
                    query = query.where("mime_type", ">=", "audio/").where("mime_type", "<", "audio0")

        # Apply sorting
        if sort_by_timestamp:
            query = query.order_by("timestamp", direction=firestore.Query.DESCENDING)

        # True Server-Side Pagination using offset and limit
        skip_count = max(0, (page - 1) * media_per_page)
        
        # We need to fetch slightly more if we couldn't push all filters to the DB
        # If there are multiple types or error filters, we fetch a bit more to ensure we can fill the page.
        # But if the query is simple, we fetch exactly what we need.
        requires_client_filter = (
            (type_filters and "all" not in type_filters and len(type_filters) > 1) or
            (error_filter != "all")
        )
        
        if requires_client_filter:
            # Fallback for complex combinations: fetch a batch, filter, and manually offset
            # We optimize this by still trying to push as much to DB as possible.
            query = query.limit(500) 
            all_docs = list(query.stream())
        else:
            # Fully optimized DB-side pagination
            query = query.offset(skip_count).limit(media_per_page)
            all_docs = list(query.stream())
        ...
        for doc in all_docs:
            ...
            # Client-side validation (acts as secondary pass for complex multi-filters)
            if requires_client_filter:
                if type_filters and "all" not in type_filters:
                    passes = False
                    for t in type_filters:
                        if (t == "videos" and mime_type.startswith("video/")) or \
                           (t == "images" and mime_type.startswith("image/")) or \
                           (t in ("music", "audio") and mime_type.startswith("audio/")):
                            passes = True
                    if not passes:
                        continue
                
                if error_filter == "no_errors" and error_message_present:
                    continue
                if error_filter == "only_errors" and not error_message_present:
                    continue
        ...
        # If we had to manually filter, apply the python-side slicing
        if requires_client_filter:
            all_fetched_items = all_fetched_items[skip_count : skip_count + media_per_page]

        logger.info(f"[get_media_for_page] Returning {len(all_fetched_items)} items for page {page}")
