...
def on_click_generate_images(e: me.ClickEvent):
    ...
    amount = max(1, int(state.imagen_image_count))
    if not check_quota("image", amount):
        state.dialog_message = f"Quota exceeded. You do not have enough quota to generate {amount} image(s). Please contact your admin for this."
        state.show_dialog = True
