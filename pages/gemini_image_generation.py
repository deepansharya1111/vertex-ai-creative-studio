...
def _generate_and_save(base_prompt: str, input_gcs_uris: list[str]):
    ...
    state.suggested_transformations = []
    amount = max(1, state.num_images_to_generate)
    from state.state import check_quota
    if not check_quota("image", amount):
        yield from show_snackbar(state, f"Quota exceeded. You do not have enough quota to generate {amount} image(s). Please contact your admin for this.")
        return

    try:
        from services.access_service import increment_user_usage
        increment_user_usage(app_state.user_email, "image", amount)
        app_state.image_usage += amount
    except Exception as e:
    ...
    try:
        ...
        try:
            ...
            decrement_user_usage(app_state.user_email, "image", amount)
            app_state.image_usage = max(0, app_state.image_usage - amount)
